#!/bin/bash
# installs OpenVPN on Debian (with 2FA via Google Authenticator)
# Code taken from this repo: https://github.com/angristan/openvpn-install
# and adjusted specifically for Debian (using all default encryption settings)
...
function installOpenVPN() {
    ...
    if [[ ! -e /etc/openvpn/server.conf ]]; then
        ...
        apt-get update
        apt-get install -y cmake git liblz4-dev liblzo2-dev libpam-dev libssl-dev pkg-config qrencode wget ca-certificates curl tar iptables-persistent openvpn
